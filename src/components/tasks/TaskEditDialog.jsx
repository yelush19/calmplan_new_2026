    ...
    <Dialog open={open} onOpenChange={(val) => { if (!val) onClose(); }}>
      <DialogContent className="sm:max-w-[520px] max-h-[85vh] overflow-y-auto" dir="rtl">
        ...
        <div className="space-y-4 py-2">
                    ...
                    <div key={st.id} className="flex items-center gap-2 p-1.5 rounded bg-[#F5F5F5] group">
                      <button onClick={() => handleToggleSubTask(st.id)} className="shrink-0">
                        {st.done ? (
                          ...
                          <Square className="w-4 h-4 text-[#546E7A]" />
                        )}
                      </button>
                      {stPri && (
                        <stPri.icon className={`w-3 h-3 shrink-0 ${stPri.color}`} />
                      )}
                      <span className={`text-xs flex-1 ${st.done ? 'line-through text-[#546E7A]' : 'text-[#263238]'}`}>
                        {st.title}
                      </span>
                      {st.due_time && (
          ...
          <div className="space-y-2">
            ...
            {(() => {
              ...
              if (childTasks.length === 0) return (
                <p className="text-xs text-[#546E7A] text-center py-2">אין תתי-משימות מלאות</p>
              );
              return (
          ...
          {task.client_name && (
            <div className="text-xs text-[#455A64] bg-[#F5F5F5] rounded p-2">
              לקוח: <span className="font-medium text-[#263238]">{task.client_name}</span>
            </div>
          )}
