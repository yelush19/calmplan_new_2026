...
export default function ClientRecurringTasks({ onGenerateComplete }) {
  ...
  // Find ghost tasks: auto-generated tasks for months that weren't injected
  const ghostTasks = useMemo(() => {
    ...
    return existingTasks.filter(t => {
      if (!t.due_date || t.due_date < nextMonthStart) return false;
      // Auto-generated by cascade (has workflow_phase or is_collector or waiting_for_materials)
      if (t.workflow_phase || t.is_collector || t.status === 'waiting_for_materials') return true;
      return false;
    });
  }, [existingTasks]);
          ...
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-amber-50 border-2 border-amber-200">
            ...
            {ghostTasks.length > 0 && (
              ...
              >
                ...
                נקה {ghostTasks.length} רפאים (חודשים עתידיים)
              </Button>
            )}
          </div>
