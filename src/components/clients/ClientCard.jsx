...
import { Phone, Mail, Edit, Building, User, DollarSign, Trash2, UserCheck, FileText, ChevronDown, ChevronUp, CheckSquare, Users, Briefcase, Calendar, MoreVertical, CheckCircle, Clock, Heart, AlertCircle, BookUser } from 'lucide-react';
...
export default function ClientCard({ client, onEdit, onSelectAccounts, onSelectCollections, onSelectContracts, onDelete, onSelectTasks, onSelectServiceProviders }) {
  ...
  return (
    <Card className="w-full transform transition-all duration-300 shadow-sm hover:shadow-lg border border-neutral-light/80 bg-white flex flex-col group">
      <CardHeader className="pb-3 flex-shrink-0">
        <div className="flex justify-between items-start gap-2">
            <CardTitle className="text-lg font-bold flex items-center gap-2 text-gray-800 group-hover:text-[#657453] transition-colors leading-tight min-w-0">
      ...
      <CardFooter className="grid grid-cols-4 md:grid-cols-7 gap-1 p-2 border-t bg-neutral-light/20 flex-shrink-0">
          ...
          <Button variant="ghost" size="sm" onClick={() => onSelectServiceProviders && onSelectServiceProviders(client)} className="h-8 text-xs text-neutral-medium hover:bg-neutral-light/50 hover:text-litay-accent">
              <BookUser className="w-3 h-3 ml-1" />
              ספקים
          </Button>
