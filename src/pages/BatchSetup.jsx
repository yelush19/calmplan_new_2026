...
import { Save, CheckCircle, AlertTriangle, Loader2, Zap, ChevronDown, ChevronUp, ChevronRight } from "lucide-react";
...
export default function BatchSetupPage() {
  ...
  const [collapsedTiers, setCollapsedTiers] = useState(new Set(['0', '1', '2', '3']));
  ...
  const groupedClients = useMemo(() => {
    const groups = {};
    clients.forEach((client, originalIdx) => {
      const merged = getMergedClient(client);
      const tier = String(computeComplexityTier(merged));
      if (!groups[tier]) groups[tier] = [];
      groups[tier].push({ client, originalIdx });
    });
    return Object.entries(groups).sort(([a], [b]) => Number(a) - Number(b));
  }, [clients, editBuffer, getMergedClient]);
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        <Button variant="outline" size="sm" onClick={() => {
          if (allExpanded) {
            setCollapsedTiers(new Set(['0', '1', '2', '3']));
      ...
      <div className="rounded-xl border dark:border-gray-700 bg-card shadow-sm">
        <Table>
          ...
          <TableBody>
            {groupedClients.map(([tierKey, entries]) => {
              const tierNum = Number(tierKey);
              const tierData = COMPLEXITY_TIERS[tierNum];
              const isCollapsed = collapsedTiers.has(tierKey);
              ...
              return (
                <React.Fragment key={`tier-${tierKey}`}>
                  ...
                  {!isCollapsed && entries.map(({ client, originalIdx: rowIdx }) => {
                    const merged = getMergedClient(client);
