...
export const cleanupGhostTasks = async ({ dryRun = true } = {}) => {
  ...
  try {
    ...
    // Detect ghost tasks — ONLY by reporting_month, NEVER by due_date
    // (March reporting has April due_dates — that's correct, not a ghost!)
    const now = new Date();
    ...
    for (const task of allTasks) {
      ...
      // Ghost rule: reporting_month AFTER current reporting period (numeric comparison)
      if (task.reporting_month) {
        const rmParts = task.reporting_month.split('-');
        ...
        if (rmYear > prevMonthYear || (rmYear === prevMonthYear && rmMonth > prevMonth)) {
          ghostTasks.push({
            ...
            status: task.status,
            reason: `חודש דיווח עתידי: ${task.reporting_month} (נוכחי: ${prevMonthYear}-${pad(prevMonth)})`,
          });
          continue;
        }
      }
