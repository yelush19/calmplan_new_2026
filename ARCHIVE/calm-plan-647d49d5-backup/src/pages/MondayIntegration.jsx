...
import { mondayAdditionalServicesAutomation } from "@/api/functions";
...
const boardCategories = {
  ...
  monthly_report: { label: 'דיווחים חודשיים', icon: Calendar, color: 'text-indigo-600', description: 'לוחות דיווח חודשיים (12 לוחות לשנה)' },
  ...
  additional_services: { label: 'שירותים נוספים', icon: Plus, color: 'text-emerald-600', description: 'לוחות חודשיים לשירותים נוספים (תלושים, מס״ב, דיווחים)' },
      ...
      <CardContent className="pt-0 flex-grow flex flex-col">
        <div className="space-y-4 flex-grow">
          ...
          <div className="flex gap-2 pt-4 mt-auto">
            ...
            {['clients', 'reports_main', 'reports_126_856_2025', 'reports_126_856_2024', 'weekly_tasks', 'balance_sheets', 'family_tasks', 'wellbeing', 'weekly_planning', 'therapists', 'reconciliations', 'client_accounts', 'monthly_report', 'additional_services'].includes(board.type) && (
              <Button
                size="sm"
                variant="destructive"
...
export default function MondayIntegrationPage() {
  ...
  const [isCreatingAdditionalServicesBoards, setIsCreatingAdditionalServicesBoards] = useState(false);
  const [additionalServicesBoardsYear, setAdditionalServicesBoardsYear] = useState(2025);
  const [additionalServicesBoardsStartMonth, setAdditionalServicesBoardsStartMonth] = useState(10);
  const [additionalServicesBoardsResult, setAdditionalServicesBoardsResult] = useState(null);
  const [isCreatingAdditionalServicesTasks, setIsCreatingAdditionalServicesTasks] = useState(false);
  const [additionalServicesTasksMonth, setAdditionalServicesTasksMonth] = useState(new Date().getMonth() + 1);
  const [additionalServicesTasksYear, setAdditionalServicesTasksYear] = useState(new Date().getFullYear());
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [dashboardsData, clientsData, tasksData, recsData, schedulesData, clientAccountsData, therapistsData, additionalServicesData] = await Promise.all([
        ...
        base44.entities.AdditionalService.list().catch(e => { console.warn('Failed to load additional services:', e); return []; })
      ]);
        ...
        if (config.monday_board_id) {
            ...
            switch(config.type) {
                ...
                case 'additional_services':
                    const boardAdditionalServices = (additionalServicesData || []).filter(s => String(s.monday_board_id) === boardIdStr);
                    count = boardAdditionalServices.length;
                    if(count > 0) lastSync = Math.max(...boardAdditionalServices.map(s => new Date(s.updated_date).getTime()));
                    break;
            }
  ...
  const handleCreateAdditionalServicesBoards = async () => {
    if (!window.confirm(`האם ליצור לוחות שירותים נוספים עבור חודשים ${additionalServicesBoardsStartMonth}-12/${additionalServicesBoardsYear}?`)) {
      return;
    }

    setIsCreatingAdditionalServicesBoards(true);
    setAdditionalServicesBoardsResult(null);
    
    try {
      const response = await mondayApi({ 
        action: 'createAdditionalServicesBoards', 
        year: additionalServicesBoardsYear,
        month: additionalServicesBoardsStartMonth
      });
      
      if (response.data.success) {
        setAdditionalServicesBoardsResult({
          type: 'success',
          message: `✅ נוצרו ${response.data.createdBoards.length} לוחות שירותים נוספים!`,
          boards: response.data.createdBoards
        });
        ...
        setAdditionalServicesBoardsResult({
          type: 'error',
          message: 'שגיאה ביצירת לוחות שירותים נוספים'
        });
      }
    } catch (error) {
      console.error("Error creating additional services boards:", error);
      setAdditionalServicesBoardsResult({
        type: 'error',
        message: `שגיאה: ${error?.response?.data?.error || error.message}`
      });
    } finally {
      setIsCreatingAdditionalServicesBoards(false);
    }
  };
  ...
  const handleSyncBoard = async (board, { shouldReloadData = true } = {}) => {
    ...
    try {
      ...
      const actions = {
        ...
        monthly_report: 'syncTasks',
        additional_services: 'syncAdditionalServices'
      };
  ...
  const handleSyncAll = async () => {
    ...
    const syncableBoards = boardConfigs.filter(d =>
      d.monday_board_id &&
      ['clients', 'reports_main', 'reports_126_856_2025', 'reports_126_856_2024', 'weekly_tasks', 'balance_sheets', 'reconciliations', 'client_accounts', 'weekly_planning', 'family_tasks', 'wellbeing', 'therapists', 'monthly_report', 'additional_services'].includes(d.type)
    );
  ...
  const handleQuickClientSync = async () => {
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        setLogs(prev => ({ 
          ...prev, 
          clients: [...(prev.clients || []), `הסיום: נוצרו ${created}, עודכנו ${updated}`] 
        }));
  ...
  const handleCreateAdditionalServicesTasks = async () => {
    if (!window.confirm(`האם ליצור משימות שירותים נוספים עבור חודש ${String(additionalServicesTasksMonth).padStart(2,'0')}.${additionalServicesTasksYear}?`)) {
      return;
    }

    setIsCreatingAdditionalServicesTasks(true);
    setError(null);
    
    try {
      const response = await mondayAdditionalServicesAutomation({ 
        targetYear: additionalServicesTasksYear, 
        targetMonth: additionalServicesTasksMonth 
      });
      
      const res = response?.data;
      if (res?.success) {
        alert(res.message || `נוצרו משימות שירותים נוספים לחודש ${String(additionalServicesTasksMonth).padStart(2,'0')}.${additionalServicesTasksYear}`);
        setLastSyncTime(new Date());
      } else {
        throw new Error(res?.error || 'שגיאה לא ידועה ביצירת משימות שירותים נוספים');
      }
    } catch (err) {
      console.error('Create additional services tasks error:', err);
      setError(`שגיאה ביצירת משימות: ${err?.response?.data?.error || err.message}`);
    } finally {
      setIsCreatingAdditionalServicesTasks(false);
    }
  };
  ...
  const configurableBoards = boardConfigs.filter(d => ['clients', 'reports_main', 'reports_126_856_2025', 'reports_126_856_2024', 'weekly_tasks', 'balance_sheets', 'reconciliations', 'client_accounts', 'weekly_planning', 'family_tasks', 'wellbeing', 'therapists', 'monthly_report', 'additional_services'].includes(d.type) && d.monday_board_id).length;
  ...
  return (
    <div className="space-y-8 p-1 md:p-4">
      ...
      {/* Create Additional Services Boards Section */}
      <div className="max-w-3xl mx-auto w-full">
        <Card className="border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-pink-50">
          <CardContent className="p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-semibold text-purple-900">יצירת לוחות שירותים נוספים</h3>
                <p className="text-sm text-purple-700">צור לוחות חודשיים לניהול שירותים נוספים (תלושים, מס״ב, דיווחים)</p>
              </div>
            </div>

            {additionalServicesBoardsResult && (
              <div className={`p-4 rounded-lg border ${
                additionalServicesBoardsResult.type === 'success' 
                  ? 'bg-green-50 border-green-200 text-green-800' 
                  : 'bg-red-50 border-red-200 text-red-800'
              }`}>
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <p className="font-medium mb-2">{additionalServicesBoardsResult.message}</p>
                    
                    {additionalServicesBoardsResult.boards && (
                      <div className="mt-3 space-y-1">
                        <p className="text-sm font-semibold">לוחות שנוצרו:</p>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                          {additionalServicesBoardsResult.boards.map(board => (
                            <div key={board.month} className="bg-white/50 p-2 rounded">
                              {board.monthName}
                            </div>
                          ))}
                        </div>
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => setAdditionalServicesBoardsResult(null)}
                    className="flex-shrink-0"
                  >
                    <X className="w-4 h-4" />
                  </Button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
              <div>
                ...
                <Input
                  ...
                  min={2024}
                  max={2030}
                  value={additionalServicesBoardsYear}
                  onChange={(e) => setAdditionalServicesBoardsYear(Number(e.target.value))}
                  className="h-10 bg-white"
                />
              </div>
              <div>
                ...
                <Select 
                  value={String(additionalServicesBoardsStartMonth)} 
                  onValueChange={(val) => setAdditionalServicesBoardsStartMonth(Number(val))}
                >
            ...
            <Button
              onClick={handleCreateAdditionalServicesBoards}
              disabled={isCreatingAdditionalServicesBoards}
              className="bg-purple-600 hover:bg-purple-700 text-white h-10"
              size="lg"
            >
              {isCreatingAdditionalServicesBoards ? (
                <>
                  <RefreshCw className="w-5 h-5 ml-2 animate-spin" />
                  יוצר לוחות...
                </>
            ...
            <div className="bg-purple-100/50 p-3 rounded-lg">
              <p className="text-sm text-purple-800 font-semibold mb-2">
                הלוחות יכללו 8 קבוצות שירות:
              </p>
              <ul className="text-sm text-purple-700 space-y-1 mr-4">
                <li>• משלוח תלושים</li>
                <li>• מס"ב שכר</li>
                <li>• פנסיה למתפעל</li>
                <li>• מס"ב סוציאליות</li>
                <li>• מס"ב רשויות</li>
                <li>• דיווח לטמל-אחיד</li>
                <li>• מס"ב ספקים סייקל-1</li>
                <li>• מס"ב ספקים סייקל-2</li>
              </ul>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 p-3 rounded-lg">
              <p className="text-sm text-amber-800 flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" />
                <span>
                  <strong>שים לב:</strong> הלוחות ייווצרו עם קישור ללוח הלקוחות. 
                  תוכל להוסיף משימות ידנית או לפתח אוטומציה עתידית על בסיס עמודת "שירותים נוספים" בלוח הלקוחות.
                </span>
              </p>
            </div>
          </CardContent>
      ...
      {/* Create Additional Services Tasks Section */}
      <div className="max-w-3xl mx-auto w-full">
        <Card className="mt-6 border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50">
          <CardContent className="p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-semibold text-emerald-900">יצירת משימות שירותים נוספים</h3>
                <p className="text-sm text-emerald-700">צור משימות אוטומטית עבור לקוחות עם שירותים נוספים</p>
              </div>
            </div>

            <div className="bg-emerald-100/50 p-3 rounded-lg">
              <p className="text-sm text-emerald-800 font-semibold mb-2">
                המערכת תיצור משימות עבור:
              </p>
              <ul className="text-sm text-emerald-700 space-y-1 mr-4">
                <li>• משלוח תלושים (חודשי)</li>
                <li>• מס"ב שכר (חודשי)</li>
                <li>• פנסיה למתפעל (חודשי)</li>
                <li>• מס"ב סוציאליות (חודשי)</li>
                <li>• מס"ב רשויות (חודשי)</li>
                <li>• דיווח לטמל-אחיד (חודשי)</li>
                <li>• מס"ב ספקים סייקל-1 (יעד: 15 לחודש)</li>
                <li>• מס"ב ספקים סייקל-2 (יעד: 28 לחודש)</li>
              </ul>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
              <div>
                <label className="text-xs text-emerald-700 font-medium">חודש</label>
                <Input
                  type="number"
                  min={1}
                  max={12}
                  value={additionalServicesTasksMonth}
                  onChange={(e) => setAdditionalServicesTasksMonth(Math.max(1, Math.min(12, Number(e.target.value) || 1)))}
                  className="h-9 bg-white"
                />
              </div>
              <div>
                <label className="text-xs text-emerald-700 font-medium">שנה</label>
                <Input
                  type="number"
                  min={2024}
                  max={2030}
                  value={additionalServicesTasksYear}
                  onChange={(e) => setAdditionalServicesTasksYear(Math.max(2024, Math.min(2030, Number(e.target.value) || 2025)))}
                  className="h-9 bg-white"
                />
              </div>
              <div className="col-span-2">
                <Button
                  onClick={handleCreateAdditionalServicesTasks}
                  disabled={isCreatingAdditionalServicesTasks}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 h-9"
                >
                  <Calendar className={`w-4 h-4 ml-2 ${isCreatingAdditionalServicesTasks ? 'animate-spin' : ''}`} />
                  {isCreatingAdditionalServicesTasks ? 'יוצר משימות...' : `צור משימות לחודש ${String(additionalServicesTasksMonth).padStart(2,'0')}.${additionalServicesTasksYear}`}
                </Button>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 p-3 rounded-lg">
              <p className="text-sm text-amber-800 flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" />
                <span>
                  <strong>חשוב:</strong> ודא שלוח השירותים הנוספים לחודש זה כבר נוצר. 
                  המערכת תקרא אוטומטית את הלקוחות עם עמודת "סוג שירות נוסף" מלוח הלקוחות 
                  ותיצור משימות בהתאם בקבוצות המתאימות.
                </span>
              </p>
            </div>
          </CardContent>
